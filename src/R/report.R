...
detectPolypharmacy <- function(atc_entry, medication = "antidepressants", type = "all") {
  ...
  atc_list <- strsplit(atc_entry, split = ",") |>
    ...
    unlist() |>
    unique() # Prevent counting the same medication more than once
...
addYearToResPoly <- function(tbl_res_poly, start_year = 2018) {
  #' Add Year
  #'
  #' Add year to a table containing polypharmacy statistics for multiple years.
  #' Usually, the statistics is ordered by year ascendingly. It means that the
  #' first row represents the earliest year in the dataset. Since the table
  #' summary contains three measures per year, the added year is repeated three
  #' times, resulting in $n \times 3$ row, where $n$ is the number of years
  #' added.
  #'
  #' @param tbl_res_poly A tidy data frame representing the summary statistics
  #' of polypharmacy
  #' @param start_year The starting year to annotate
  #' @return An annotated tidy data frame of the summary statistics

  # Set the number of repetition and number of years
  reps  <- 3
  nyear <- round(nrow(tbl_res_poly) / reps)
  years <- seq(start_year, length.out = nyear) |> rep(each = reps)

  # Annotate years to the
  tbl  <- tbl_res_poly |> tibble::add_column("year" = years, .after = 1)

  return(tbl)
}

catResPoly <- function(..., start_year = 2018) {
  #' Concatenate Polypharmacy Summary Tables
  #'
  #' Concatenate multiple polypharmacy summary tables generated by
  #' `summarizePolypharmacy`
  #'
  #' @param ... Multiple tidy data frames representing the summary
  #' statistics of polypharmacy
  #' @param start_year The starting year to annotate
  #' @return A concatenated tidy data frame of the summary statistics of
  #' polypharmacy

  tbls <- list(...) |> lapply(addYearToResPoly, start_year = start_year)
