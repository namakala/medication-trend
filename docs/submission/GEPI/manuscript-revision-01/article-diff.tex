...
%DIF ADD article.tex    Mon Dec 15 13:46:05 2025
%DIF 2a2
...
relative to the expected value \DIFdelbegin \DIFdel{\(\frac{1}{n}\)}\DIFdelend \DIFaddbegin \DIFadd{\(C_e = \frac{1}{n}\)}\DIFaddend , based on the null
model of uniform connectivity (see ``Determining relative importance'').
...
supplementary file section S1\DIFdelbegin \DIFdel{.1}\DIFdelend \DIFaddbegin \DIFadd{.2}\DIFaddend .
...
\DIFaddbegin \DIFadd{\(C_e\) }\DIFaddend for each node in a uniformly connected network is
\(\frac{1}{n}\), where \(n\) is \DIFaddbegin \DIFadd{24, representing }\DIFaddend the total number of
nodes. In such a network, each node has an equal probability of being
connected to any other, and thus no node is more ``influential'' than
another. This expected value\DIFdelbegin \DIFdel{value, \(\frac{1}{n}\)}\DIFdelend , \DIFaddbegin \DIFadd{\(\frac{1}{24}\), }\DIFaddend serves as a baseline
under a null model of uniform connectivity. Assuming eigenvector
centrality scores approximately follow a normal distribution, we applied
a one-sample Student's T-test with Bonferroni correction to assess how
much each eigenvector centrality differs from the expected values. Nodes
with \(c_i\) greater than \DIFdelbegin \DIFdel{\(\frac{1}{n}\) }\DIFdelend \DIFaddbegin \DIFadd{\(\frac{1}{24}\) }\DIFaddend were categorized as having
high eigenvector centrality, and those below the expected value were
considered low.

\DIFaddbegin \subsubsection{\DIFadd{Subgroup analysis}}\label{subgroup-analysis}

\DIFadd{Subgroup analyses were conducted by first categorizing the population
into two age groups: \(< 65\) and \(\geq 65\) years \mbox{%DIFAUXCMD
\citep{Uddin2022}}\hskip0pt%DIFAUXCMD
.
The second categorization was based on general polypharmacy, defined a
receiving \(\geq 5\) medications for at least 30 consecutive days
\mbox{%DIFAUXCMD
\citep{nicholson2024prevalence}}\hskip0pt%DIFAUXCMD
. These subgroup classifications were
combined to form three comparison pairs: \(< 65\) vs.~\(\geq 65\) years;
non-polypharmacy vs.~polypharmacy; and the non-polypharmacy group aged
\(< 65\) years vs.~the polypharmacy group aged \(\geq 65\) years. A DPN
was generated for each subgroup to calculate its respective eigenvector
centrality.
}
...
\DIFadd{The eigenvector centrality patterns shown in Figure~\ref{fig-hi-eigen}
also highlight the distribution of several subgroups, each distinguished
by color. Most subgroup distribution largely overlapped with the general
population, represented by the gray density curves. However, two
subgroups consistently diverged from this overall pattern: the
polypharmacy group and the subset of polypharmacy patients aged
\(\geq 65\) years. These subgroups exhibited higher eigenvector
centralities across four medication classes, demonstrating a more
tightly connected co-prescription patterns within these groups. In
addition, the \(\geq 65\) years subgroup showed elevated eigenvector
centralities in three medication classes, although its magnitude was
smaller than that observed in the broader polypharmacy population.
}
...
co-prescription patterns \DIFdelbeginFL \DIFdelFL{on antidepressants }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for antidepressnts }\DIFaddendFL and anxiolytics\DIFdelbeginFL \DIFdelFL{dispensing}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. The
vertical dashed line indicates the expected eigenvector centrality,
calculated as 1/24. This line provides a reference for identifying
