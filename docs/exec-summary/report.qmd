...
**Methods:**
Exploratory time-series analysis was performed using University of Groningen prescription database, IADB.nl. Data of patients with at least one prescription of psychopharmaca from 2018 to 2022 was extracted. Prescription was evaluated daily using anatomical therapeutic chemical (ATC) classification, where psychopharmaca is evaluated at ATC level 3, drugs for nervous system at ATC level 2, and other medications at ATC level 1, totalled to 24 classes of medication. A medication-claim network was constructed as an undirected graph to capture dose-adjusted co-prescription. Eigenvector centrality was computed as a measure of relative nodal importance, which reflects the degree of medication concurrence in a network. The number of medication claims, number of patients, claim-to-patient ratio, and eigenvector centrality were then extracted as a weekly-aggregated time-series data. Singular spectrum analysis was performed to decompose the data into its constituting trend, periodicity, and white noises.

**Results:**
Antidepressants (ATC `N06A`), antipsychotics (`N05A`), anxiolytics (`N05B`), and hypnotics (`N05C`) are psychopharmaca with high eigenvector centrality. These classes of medication are likely to be co-prescribed alongside other medications, including medications for alimentary and metabolism (`A01-A16`), cardiovascular (`C01-C10`), analgesics (`N02`), and respiratory (`R01-R07`). Only antidepressants and hypnotics have a positive time trend for both eigenvector centrality and actual number of claims.

**Discussion:**
Eight medications, four of which are psychopharmaca, have a high eigenvector centrality, implying its concurrence on a population level. Positive time trends in both eigenvector centrality and claims suggest that increasing uses of antidepressants and hypnotics are related to co-prescription with other medication classes. Constructing a medication claim network enriches the exploratory analysis procedure on the population level.
