...
The relational characteristics in health research variables has fueled the growing interest of implementing network analysis, making it an indispensable research framework in medicine and epidemiology [@Luke2007]. Past implementations of network analysis in public health mostly focused on transmission network, social network, and organizational network. Only recently, @Cavallo2012 demonstrated a novel framework to describe medication uses on a population level, i.e. drug prescription network (DPN), a term which later is coined by @Bazzoni2015. Network analysis offers an approach to capture the relationship among building blocks of a complex system. From a health system standpoint, polypharmacy is a prescription of multiple drug combinations which can be further evaluated as a network [@Miglio2021]. DPN is a complex and dynamic system that may change overtime, specifically also in drug prescribed for mental disorders.
